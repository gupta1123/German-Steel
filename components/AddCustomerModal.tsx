...
type TabId = 'account' | 'address' | 'commercial' | 'contacts' | 'brands' | 'review';

const TAB_ORDER: TabId[] = ['account', 'address', 'commercial', 'contacts', 'brands', 'review'];
...
const stateOptions: SearchableOption[] = INDIAN_STATES.map((s) => ({ value: s, label: s }));
...
const AddCustomerModal: React.FC<AddCustomerModalProps> = ({
  ...
  const formSessionIdRef = useRef<string>('');
  ...
  const resetForm = () => {
    ...
    const fresh = createEmptyRetailAccountDraft(employeeId);
    if (initialGroupId) fresh.clientGroupId = initialGroupId;
    setDraft(fresh);
  ...
  useEffect(() => {
    ...
    const loadMasters = async () => {
      ...
      else warnings.push('Client groups could not be loaded; a Standalone group will be auto-created on save.');
      ...
      try {
        const brandResult = await RetailAPI.getCompetitorBrands(token);
        setBrands(brandResult);
      } catch {
        setBrands([]);
        warnings.push('Competitor brands could not be loaded; you can add them after creation.');
      }
  ...
  const handleCreateBrand = async () => {
    ...
    try {
      const brandId = await RetailAPI.createCompetitorBrand(name, token);
      setBrands((prev) => [...prev, { id: brandId, name }]);
      setSelectedBrandIds((prev) => [...prev, brandId]);
      setNewBrandName('');
      toast.success(`Brand "${name}" created and selected.`);
    } catch (error) {
      toast.error(getErrorMessage(error, 'Could not create brand.'));
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      let groupId = draft.clientGroupId ? Number(draft.clientGroupId) : null;
      if (!groupId) {
        const autoGroupName = `${draft.accountName.trim()} - Standalone`.slice(0, 200);
        try {
          groupId = await RetailAPI.createClientGroup(
            { groupName: autoGroupName, groupType: 'STANDALONE', notes: null, active: true },
            token,
          );
          setGroups((prev) => [...prev, { id: groupId as number, groupName: autoGroupName, groupType: 'STANDALONE', active: true }]);
        } catch (groupError) {
          const existing = groups.find((g) => g.groupName.toLowerCase() === autoGroupName.toLowerCase());
          if (existing) {
            groupId = existing.id;
          } else {
            try {
              const refreshed = await RetailAPI.getClientGroups(token);
              setGroups(refreshed.filter((g) => g.active !== false));
              const match = refreshed.find((g) => g.groupName.toLowerCase() === autoGroupName.toLowerCase());
      ...
      const accountId = await RetailAPI.createAccount(
        { ...buildRetailAccountPayload(draft), clientGroupId: groupId },
        token,
      );
      ...
      if (failures.length > 0) {
        ...
        router.push(`/dashboard/customers/${accountId}`);
        onClose();
      }
    } catch (error) {
  ...
  const selectedGroup = groupOptions.find((option) => option.value === draft.clientGroupId)?.label
    || (draft.accountName.trim() ? `${draft.accountName.trim()} - Standalone (auto)` : 'Auto-standalone on save');
    ...
    <Sheet open={isOpen} onOpenChange={(open) => !open && handleCloseRequest()}>
      <SheetContent className="flex w-full flex-col sm:max-w-2xl">
        <SheetHeader className="border-b pb-4">
          <SheetTitle>Create Retail Customer</SheetTitle>
        </SheetHeader>
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as TabId)} className="flex min-h-0 flex-1 flex-col">
          <div className="border-b px-1 pt-2">
            <TabsList className="flex h-auto w-full flex-wrap gap-3 px-1 py-1">
              {TAB_ORDER.map((tab) => (
                <TabsTrigger key={tab} value={tab} className="text-xs px-4 py-1.5">{TAB_LABELS[tab]}</TabsTrigger>
              ))}
            </TabsList>
          </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-1 py-4">
            ...
            <TabsContent value="account" className="m-0 space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Client Group" hint="Leave empty to auto-create a Standalone group named after this firm. Select a Family / Corporate group to link branches.">
                  <SearchableSelect
            ...
            <TabsContent value="address" className="m-0 space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="State" required>
                  <SearchableSelect
                    triggerId="retail-state"
                    value={draft.addressState}
                    options={stateOptions}
                    onSelect={(option) => updateDraft('addressState', option?.value ?? '')}
            ...
            <TabsContent value="commercial" className="m-0 space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Declared Monthly Sales (MT/month)">
                ...
                <Field label="Client Tier" required>
            ...
            <TabsContent value="brands" className="m-0 space-y-5">
              <div className="flex gap-2">
                ...
                />
                <Button type="button" variant="outline" onClick={handleCreateBrand} disabled={!newBrandName.trim() || isCreatingBrand}>
              ...
              {isLoadingMasters && brands.length === 0 ? (
                ...
                <p className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">No brands available yet. Use the field above to add one.</p>
              ) : (
              ...
              <Field label="Brand Usage Remarks">
                <Textarea
                  value={brandRemarks}
                  onChange={(event) => { touch(); setBrandRemarks(event.target.value); }}
            ...
            <TabsContent value="review" className="m-0 space-y-4">
              {createdAccountId != null ? (
                <div className={`rounded-xl border-2 p-6 ${postCreateFailures.length ? 'border-amber-300 bg-amber-50/60' : 'border-emerald-300 bg-emerald-50/60'}`}>
                  <div className="flex items-start gap-4">
                    <CheckCircle2 className={`mt-0.5 h-6 w-6 shrink-0 ${postCreateFailures.length ? 'text-amber-600' : 'text-emerald-600'}`} />
                    <div className="space-y-1">
                      <p className="text-base font-semibold">Account #{createdAccountId} created</p>
                      {postCreateFailures.length > 0 ? (
                        <>
                          <p className="text-sm text-muted-foreground">Account is saved. These linked records need follow-up:</p>
                          <ul className="mt-2 space-y-1 text-sm">
                            {postCreateFailures.map((f) => <li key={f} className="text-amber-800">- {f}</li>)}
                          </ul>
                        </>
                      ) : (
                        <p className="text-sm text-muted-foreground">All contacts and brands linked successfully.</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-0 rounded-xl border divide-y">
                  ...
                  <div className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                      ...
                      <div><span className="text-muted-foreground">GSTIN</span><p className="font-medium mt-0.5 font-mono">{draft.gstNumber || '—'}</p></div>
                      <div><span className="text-muted-foreground">Owner</span><p className="font-medium mt-0.5">{selectedOwner}</p></div>
                      <div><span className="text-muted-foreground">Group</span><p className="font-medium mt-0.5">{selectedGroup}</p></div>
                      <div><span className="text-muted-foreground">Status</span><p className="font-medium mt-0.5">Prospect</p></div>
                    </div>
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                      ...
                      <div><span className="text-muted-foreground">PIN</span><p className="font-medium mt-0.5 font-mono">{draft.pinCode || '—'}</p></div>
                      <div><span className="text-muted-foreground">Region</span><p className="font-medium mt-0.5">{derivedRegionName || 'Derived by backend'}</p></div>
                      <div><span className="text-muted-foreground">GPS</span><p className="font-medium mt-0.5 font-mono text-xs">{draft.outletLatitude && draft.outletLongitude ? `${draft.outletLatitude}, ${draft.outletLongitude}` : '—'}</p></div>
                    </div>
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                      ...
                      <div><span className="text-muted-foreground">Credit terms</span><p className="font-medium mt-0.5">{draft.creditTermsDays || '0'} days</p></div>
                      <div><span className="text-muted-foreground">Credit limit</span><p className="font-medium mt-0.5">{draft.creditLimitAmount || '0'}</p></div>
                      <div><span className="text-muted-foreground">Focus sector</span><p className="font-medium mt-0.5">{draft.focusSector.replaceAll('_', ' ')}</p></div>
                    </div>
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                      ...
                      <div className="col-span-2"><span className="text-muted-foreground">Brands</span><p className="font-medium mt-0.5">{selectedBrands.length ? selectedBrands.map((b) => b.name).join(', ') : 'None'}</p></div>
                    </div>
                  </div>
                </div>
              )}
