...
const normalizeBrandUsage = (value: unknown): RetailBrandUsage | null => {
  ...
  return { id, competitorBrandId: brandId, brandName: stringOf(item.brandName, item.competitorBrandName, brand?.name, brand?.brandName) || `Brand #${brandId || '?'}`, employeeId: numberOf(item.employeeId, item.addedByEmployeeId, item.addedBy, employee?.id), employeeName: stringOf(item.employeeName, item.addedByEmployeeName, employee?.fullName, [stringOf(employee?.firstName), stringOf(employee?.lastName)].filter(Boolean).join(' ')), remarks: stringOf(item.remarks), active: booleanOf(true, item.active), addedAt: stringOf(item.addedAt, item.createdAt), removedAt: stringOf(item.removedAt, item.endedAt) };
};
...
export const RetailAPI = {
  ...
  async getMasterContacts(token: string): Promise<RetailMasterContact[]> {
    ...
    return pageItems(response).flatMap((value) => { const item = recordOf(value); const id = numberOf(item?.id, item?.contactId); if (id == null) return []; return [{ id, firstName: stringOf(item?.firstName), lastName: stringOf(item?.lastName), mobile: stringOf(item?.mobile, item?.primaryContact), email: stringOf(item?.email), dateOfBirth: stringOf(item?.dateOfBirth), anniversaryDate: stringOf(item?.anniversaryDate), active: typeof item?.active === 'boolean' ? item.active : undefined }] });
  },
  ...
  async getNotes(accountId: number, token: string): Promise<RetailNote[]> {
    ...
    return pageItems(response).flatMap((value) => { const note = normalizeGeneric(value, (item, id) => ({ id, noteText: stringOf(item.noteText, item.text, item.note), authorName: stringOf(item.authorEmployeeName, item.authorName, item.createdByName, item.createdBy), authorEmployeeId: numberOf(item.authorEmployeeId, item.createdByEmployeeId), createdAt: stringOf(item.createdAt, item.createdDate), updatedAt: stringOf(item.updatedAt, item.updatedDate, item.lastCorrectedAt, item.createdAt) })); return note ? [note] : [] });
  },
